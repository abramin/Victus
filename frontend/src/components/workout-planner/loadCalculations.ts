...
/**
 * Load zone colors for charts and indicators.
 */
...
/**
 * Get Tailwind classes for a load zone.
 */
export function getLoadZoneClasses(zone: LoadZone): { bg: string; text: string } {
  switch (zone) {
    case 'optimal':
      return { bg: 'bg-green-600', text: 'text-green-400' };
    case 'high':
      return { bg: 'bg-amber-600', text: 'text-amber-400' };
    case 'overload':
