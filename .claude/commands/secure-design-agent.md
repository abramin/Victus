...
- Sensitive data is modeled explicitly; no echoing user input; minimize exposure in logs/errors.
- Health and biometric data is sensitive: avoid logging raw values and avoid returning more than needed.
...
3. Require idempotency and safe retries where relevant (token/session flows, external calls).
...
- Identity/token/session/authorization lifecycles (replay, confusion, bypass risks)
...
- Recommend domain primitives and invariants (where they live, when they are enforced).
...
4. Security behaviors to test: scenario names + intent.
