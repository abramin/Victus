...
- Propose idiomatic Go refactors:
  ...
  - Replace boolean soup with **small enums / intent methods** (`IsPlanned`, `IsLogged`, `CanRecalculate`).
...
- Any function with: deep nesting (3+), long switch/if ladders, repeated "special cases", many locals, or early computed state that is only used in one branch.
...
- **Hotspots (top 3):** file/function + why it is hard to read.
